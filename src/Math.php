 ...
 */
function isPositiveZero(float $x): bool
{
    // Using fdiv() to avoid a division by zero error.
    return $x == 0.0 && fdiv(1.0, $x) === INF;
}
 ...
 * It returns false for +0.0, +INF, and NaN.
 *
 * @param float $value The value to check.
 * @return bool True if the value is negative, false otherwise.
 */
function isNegative(float $value): bool
{
 ...
 */
function isPositive(float $value): bool
{
 ...
 */
function copySign(float $num, float $sign_source): float
{
 ...
 */
function intPow(int $a, int $b): int
{
    // Handle b < 0.
    if ($b < 0) {
...
/**
 * Generate a random float that is not NaN, positive or negative infinity, or negative zero.
 *
...
{
    // Generate random floats until a valid value is found.
    // It's very unlikely the loop will repeat more than once or twice.
    do {
        $bytes = random_bytes(8);
        $float = unpack('d', $bytes)[1];
    } while (!is_finite($float) || self::isNegativeZero($float));
